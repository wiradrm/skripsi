        ...
        <div class="modal-content">
            ...
            <form action="{{route('penjualan.update', $item->id)}}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group update_input_customer_hutang">
                        <label for="customer" class="col-form-label">Pelanggan</label>
                        <input @if($item->id != $item->checkLastRecord()) readonly @endif type="text" class="form-control" id="customer" name="customer" value="{{$item->customer}}">
                    </div>
                    <div class="form-group update_customer_hutang">
                        <label for="customer" class="col-form-label">Pelanggan</label>
                        <select id="customer" class="form-control"  name="id_customer">
                            @foreach($customer as $key => $data)
                            <option @if($item->id_customer === $data->id) selected @endif value="{{$data->id}}">{{$data->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select @if($item->id != $item->checkLastRecord()) readonly @endif class="form-control"  name="id_jenis_telur" id="jenis_telur">
                            ...
                            <option  @if($item->getTelurKeluar)@if($item->getTelurKeluar->id_jenis_telur == $data->id) selected @elseif($item->checkLastRecord() != $item->id) disabled @endif @endif value="{{$data->id}}">{{$data->jenis_telur}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="check_satuan">
                        <div class="form-group">
                            ...
                            <select @if($item->id != $item->checkLastRecord()) readonly @endif class="form-control satuan_update"  name="satuan" id="satuan">
                                <option @if($item->getTelurKeluar)@if($item->getTelurKeluar->satuan == "Tray") selected @elseif($item->checkLastRecord() != $item->id) disabled @endif @endif value="Tray">Tray</option>
                                <option @if($item->getTelurKeluar)@if($item->getTelurKeluar->satuan == "Butir") selected @elseif($item->checkLastRecord() != $item->id) disabled @endif @endif value="Butir">Butir</option>
                            </select>
                        </div>
                    ...
                    <div class="checkStatusPembayaran">
                        ...
                        <div class="hutang-wrapper p-4 border mb-3 updateHutang">
                            <div class="form-group">
                                ...
                                <input @if($item->id != $item->checkLastRecord()) readonly @endif type="number" class="form-control" id="pembayaran_awal" name="pembayaran_awal" value="{{$hutang->pembayaran_awal ?? ''}}">
                            </div>
                            <div class="form-group input_kadaluwarsa">
                                <label for="tgl_pelunasan" class="col-form-label">Tanggal Pelunasan</label>
                                <input @if($item->id != $item->checkLastRecord()) readonly @endif type="date" class="form-control" id="tgl_pelunasan" name="tgl_pelunasan" value="{{date('Y-m-d', strtotime($hutang->tgl_pelunasan ?? ''))}}">
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" @if($item->id != $item->checkLastRecord()) disabled @endif class="btn btn-primary">Submit</button>
                </div>
            </form>
        </div>
    </div>
