<div class="modal fade bd-example-modal-lg text-left" id="createModal" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('stock_kandang.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control"  name="id_jenis_kandang" id="jenis_kandang">
                            @foreach($jenis_kandang as $key => $item)
                            <option value="{{$item->id}}">{{$item->jenis_kandang}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control"  name="id_jenis_telur" id="jenis_telur">
                            @foreach($jenis_telur as $key => $item)
